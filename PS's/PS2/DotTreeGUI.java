...
public class DotTreeGUI extends DrawingGUI {
	...
	public void handleMousePress(int x, int y) {
		if (mode == 'a') {
			...
			if(tree == null) {
				// instantiates new tree if not generated yet
				tree = new PointQuadtree<Dot>(newDot, 0, 0, width, height);
			}
			else {
				// else inserts to current tree
				tree.insert(newDot);
			}
		}
		else if (mode == 'q') {
			...
			// TODO: YOUR CODE HERE
			// calls findInCircle helper func. and passes mouse coords
	...
	private void test2() {
		// same code as test one, but with different points/respective rectangles
		found = null;
		tree = new PointQuadtree<Dot>(new Dot(0,0), 0,0,100,100); // start with A
		...
		tree.insert(new Dot(70,70)); // G
		tree.insert(new Dot(80,80)); // H
		tree.insert(new Dot(90,90)); // I
		tree.insert(new Dot(100,100)); // J
		tree.insert(new Dot(100,10)); // K
		tree.insert(new Dot(250,250)); // L
		int bad = 0;
		bad += testFind(250,250,15,1,0,0);
		bad += testFind(600,600,10,1,0,0);
		bad += testFind(80,80,15,8,7,3);
		bad += testFind(100,100,25,12,10,2);    // find all
...
//		 TODO: YOUR CODE HERE
		// Recurse with children
		for (int q = 1; q <= 4; q++) {
			// if the tree has a child, draw said child by creating new quad tree dot
			if (tree.hasChild(q)) {
				PointQuadtree<Dot> child = tree.getChild(q);
