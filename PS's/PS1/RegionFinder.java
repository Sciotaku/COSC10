					...
					while (toVisit.size() != 0) {
						...
						for (int my = -1; my <= 1; my++) {
							for (int mx = -1; mx <= 1; mx++) {
								...
								int nx = p.x + mx;
								int ny = p.y + my;
								// If the neighbor pixel is within the image bounds, hasn't been visited yet,
								...
								if (nx >= 0 && nx < image.getWidth() && ny >= 0 && ny < image.getHeight()
									...
									// Add the neighbor pixel to the toVisit list
									toVisit.add(new Point(nx, ny));
									// Add the neighbor pixel to the current region
...
//		return true;
	...
	public ArrayList<Point> largestRegion() {
		...
		// Return a new list containing all the points in the largest region
//			ArrayList<Point> result = new ArrayList<Point>();
//			for (Point p : maxRegion) {
